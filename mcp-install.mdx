...
Install the plugin in Cowork from `https://github.com/respira-press/cowork-plugin-wordpress`, then click **Open in Cowork** on the [respira.press dashboard](https://respira.press/dashboard/mcp). A fresh Cowork chat opens with a one-time setup token pre-filled. Hit send. Claude redeems the token and writes `~/.respira/config.json` for you. No file download, no folder hunting. If you already use Respira through Claude Desktop, the config file is already on your machine and Cowork picks it up on the next chat. See the [full Cowork configuration guide](/configuration#claude-cowork).
