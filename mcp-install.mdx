...
The Cowork plugin reads `~/.respira/config.json` directly. Download the file from [your respira.press dashboard](https://respira.press/dashboard/mcp) (Cowork card → **Download Cowork config**), save it to `~/.respira/config.json`, and install the plugin in Cowork from `https://github.com/respira-press/cowork-plugin-wordpress`. If you already use Respira through Claude Desktop, the file is already on your machine and Cowork picks it up on the next chat. See the [full Cowork configuration guide](/configuration#claude-cowork).
