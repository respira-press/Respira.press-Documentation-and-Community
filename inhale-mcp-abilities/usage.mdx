...
Abilities marked **Managed** are managed by another plugin — typically the MCP Adapter's discovery ability — and cannot be toggled from Inhale.
...
When you inhale an ability annotated `destructive`, Inhale shows a single browser confirmation:

> This ability can modify content on your site. Inhale it?
...
Inhale never overrides an ability's own permission checks. Every ability still runs its own `permission_callback` before execution. Inhale controls **visibility**, not **authorization**.
...
Tick multiple rows and pick **Inhale selected** or **Exhale selected** from the bulk actions dropdown, then click **Apply**. The destructive confirmation still fires for each destructive ability in the selection.
