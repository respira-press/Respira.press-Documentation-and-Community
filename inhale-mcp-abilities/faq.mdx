...
No. [Inhale](https://www.respira.press/inhale-mcp-abilities): MCP Abilities is a settings-only utility. The whole reason it exists is to replace the PHP-filter pattern that WordPress contributor blogs documented in the months after the MCP Adapter shipped. Anything you used to do with a filter on `wp_register_ability_args` is now a checkbox.
