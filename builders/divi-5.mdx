...
This page is the technical reference. For what Respira does on Divi 5, how it connects to Claude and ChatGPT, and how every edit lands on a copy first, start with the [Divi 5 MCP server](https://www.respira.press/divi-mcp) page.
...
Respira helps you plan and validate your [Divi 5 migration](https://www.respira.press/divi-5-migration-copilot). Divi’s Migrator performs the actual conversion.
...
## Prompting for great results on Divi 5

For the full prompting guide, see [Get Great Results: How to Prompt](/prompts). Two things matter most on Divi 5:

- **Ask for native Divi modules, not HTML.** Divi 5 builds real, editable pages with native modules (heading, text, image, button, blurb). Ask for "HTML" and you get a code module your client cannot edit.
...
## New in 8.8: Divi's styling engine, presets, and the Library

Plugin 8.8.0 and 8.8.1 (with MCP server 8.3.16 and 8.3.17) came out of a customer field report that rebuilt a 16-page site and measured everything. Four changes matter for how you work on Divi 5:

**Respira-built pages now use Divi's own styling engine.** Every Divi 5 write stamps the marker Divi's dynamic-assets pipeline requires, so Divi generates CSS from module attributes and presets instead of forcing every design decision into child-theme CSS. Existing pages transition automatically on their next Respira edit, under a snapshot.

<Callout kind="info">
The first edit that activates the pipeline on a page returns a warning worth reading: Divi loads its base CSS selectively in this mode, and image wrappers can need a `display:block;width:100%` rule in the child theme if images collapse. The warning names the fix.
</Callout>

**Global presets are tools.** `list_divi_presets`, `upsert_divi_presets` (bulk, idempotent by name), `delete_divi_preset`, and `resolve_divi_preset`. A brand profile installed as presets is editable by the next person in the Visual Builder.

**The Divi Library is tools.** `save_divi_library_item` saves a page, one section of it, or a supplied structure as a real Library item. `apply_divi_library_item` places it on a page as a live-linked global reference: the page stores only the reference, Divi expands it at render, and editing the item once updates every page that carries it. Use this whenever the same section belongs on several pages.

**One payload vocabulary.** Canonical Divi 5 attribute groups are accepted on every write path, underscore spellings of module names (`contact_form`) resolve, contact forms build with real field identities, and the module list reads Divi's live registry: 112 module types on Divi 5.11, WooCommerce's 24 only when WooCommerce is active, and third-party module suites (Divi Supreme and family) listed and marked as third-party.
...
- Respira builds real, editable modules from a 112-module catalog (Divi 5.11). Ask for the catalog with `get_builder_inline_schemas`.
...
- **Native, editable module builds across a 112-module catalog (Divi 5.11).** `build_page` writes real `divi/*` modules from the catalog, discoverable through `get_builder_inline_schemas`, instead of falling back to a raw code block.
