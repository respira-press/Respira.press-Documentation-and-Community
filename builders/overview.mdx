...
| [Spectra](/builders/spectra) | Block registry | Gutenberg blocks | ✓ Full |
| [Kadence Blocks](/builders/kadence) | Block registry | Gutenberg blocks | ✓ Full |
| [GenerateBlocks](/builders/generateblocks) | Block registry | Gutenberg blocks | ✓ Full |
| [SeedProd](/builders/seedprod) | Sections / blocks | Proprietary JSON | Read only (7.1) |

### New in v7.1: four more builders + responsive in one call

Respira 7.1 adds four builders, bringing the total to 16:

- **[Spectra](/builders/spectra)**, **[Kadence Blocks](/builders/kadence)**, and **[GenerateBlocks](/builders/generateblocks)** are Gutenberg-native block libraries. They get full read and write support through Respira's Gutenberg engine, with per-page detection so mixed-builder sites resolve correctly.
- **[SeedProd](/builders/seedprod)** landing pages get read support (detection, extraction, outline). Writes are intentionally closed in 7.1 to protect the proprietary layout format.

7.1 also adds responsive inference: ask the agent to make a page responsive and it reads the desktop layout, infers tablet and mobile, and writes the breakpoints back. Available on Bricks, Elementor, Divi 4, Beaver Builder, and WPBakery.
...
All 16 supported builders have module-level support. Divi and Elementor have the most comprehensive intelligence packages. Bricks has its own deep toolset (design-token import, section presets, BEM scaffolding and linting). Flatsome has a 55-element intelligence package with nesting rules and page patterns. Spectra, Kadence, and GenerateBlocks are read and write through the Gutenberg engine; SeedProd is read only in 7.1.
