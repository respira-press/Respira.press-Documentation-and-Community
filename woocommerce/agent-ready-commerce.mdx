...
- [**woo-agent-storefront**](https://www.respira.press/skills/woo-agent-storefront) walks the whole setup: configure feeds, validate against each spec, register with each platform (including the gated OpenAI program), check AI crawler access, and demonstrate the cart-link loop.
- [**woo-catalog-perfection**](https://www.respira.press/skills/woo-catalog-perfection) runs the fixlist loop in reviewable batches until your catalog scores 90+, with a snapshot before every change.
